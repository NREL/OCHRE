...
class WaterHeater(Equipment):
    ...
    def update_external_control(self, control_signal):
        ...
        if ext_setpoint is not None:
            if ext_setpoint > self.max_temp:
                self.warn(f"Setpoint cannot exceed {self.max_temp}C. Setting setpoint to maximum value.")
    ...
    def run_duty_cycle_control(self, duty_cycles):
        ...
        if t_tank > self.max_temp:
            self.warn(f"Temperature over maximum temperature ({self.max_temp}C), forcing off")
            return "Off"
    ...
    def update_setpoint(self):
        ...
        if self.setpoint_ramp_rate and self.setpoint_temp != t_set_new:
            delta_t = self.setpoint_ramp_rate * self._dt_minutes  # in C
            self.setpoint_temp = min(
                max(t_set_new, self.setpoint_temp - delta_t),
    ...
    def solve_ideal_capacity(self):
        ...
        h_desired = (
            np.dot(
                set_states[: self.t_lower_idx + 1] - off_states[: self.t_lower_idx + 1],  # in W
    ...
    def update_internal_control(self):
        ...
        if self.use_ideal_capacity:
            if self.model.n_nodes == 1:
                ...
                h_desired = self.model.solve_for_input(
                    self.model.t_1_idx, self.model.h_1_idx, self.setpoint_temp, solve_as_output=False
                )
    ...
    def generate_results(self):
        ...
        self.add_output(results, f"{self.end_use} Delivered (W)", self.delivered_heat)

        cop = self.delivered_heat / (self.electric_kw * 1000) if self.electric_kw > 0 else 0
        self.add_output(results, f"{self.end_use} COP (-)", cop)

        self.add_output(results, f"{self.end_use} Total Sensible Heat Gain (W)", self.sensible_gain)

        self.add_output(results, f"{self.end_use} Deadband Upper Limit (C)", self.setpoint_temp)

        self.add_output(results, f"{self.end_use} Deadband Lower Limit (C)", self.setpoint_temp - self.deadband_temp)
...
class ElectricResistanceWaterHeater(WaterHeater):
    ...
    def solve_ideal_capacity(self):
        ...
        h_upper = (
            np.dot(
                set_states[: self.t_upper_idx + 1] - off_states[: self.t_upper_idx + 1],  # in W
                self.model.capacitances[: self.t_upper_idx + 1],
            )
            / self._dt_seconds
        )
...
class HeatPumpWaterHeater(ElectricResistanceWaterHeater):
    ...
    def update_inputs(self, schedule_inputs=None):
        ...
        if not self.main_simulator:
            schedule_inputs["Zone Temperature (C)"] = schedule_inputs[f"{self.zone_name} Temperature (C)"]
            schedule_inputs["Zone Wet Bulb Temperature (C)"] = schedule_inputs[
                f"{self.zone_name} Wet Bulb Temperature (C)"
            ]
    ...
    def solve_ideal_capacity(self):
        ...
        if not self.hp_only_mode and hp_mode == "Upper On":
            ...
            h_upper = (
                np.dot(
                    set_states[: self.t_upper_idx + 1] - hp_states[: self.t_upper_idx + 1],  # in W
                    self.model.capacitances[: self.t_upper_idx + 1],
                )
                / self._dt_seconds
            )
    ...
    def generate_results(self):
        ...
        self.add_output(results, f"{self.end_use} Heat Pump Max Capacity (W)", self.hp_capacity)

        if self.use_ideal_capacity:
            hp_on_frac = self.duty_cycle_by_mode["Heat Pump On"]
        else:
            hp_on_frac = 1 if "Heat Pump" in self.mode else 0
        self.add_output(results, f"{self.end_use} Heat Pump On Fraction (-)", hp_on_frac)

        self.add_output(results, f"{self.end_use} Heat Pump COP (-)", self.hp_cop)
...
class TanklessWaterHeater(WaterHeater):
    ...
    def __init__(self, **kwargs):
        kwargs.update({"use_ideal_capacity": True, "model_class": IdealWaterModel})
        super().__init__(**kwargs)
