...
ALL_SCHEDULE_NAMES = {
    ...
    for category, data in SCHEDULE_NAMES.items()
    for hpxml_name, ochre_name in data.items()
}
...
def import_weather(weather_file=None, weather_path=None, weather_station=None, weather_metadata=None, **kwargs):
    ...
    if weather_file is not None and weather_station is not None:
        if weather_station not in weather_file:
            print(
                f"WARNING: Properties file weather station ({weather_station}) may be different from weather file"
                f" used: {weather_file}"
            )
    ...
    if weather_metadata is not None:
        ...
        if missing:
            raise OCHREException(
                f"Missing required location data (must be specified in house Location parameters): {missing}"
            )
    ...
    if len(df.index.month.unique()) == 12:
        ...
        t_mains = (
            t_amb_avg
            + 6
            + tmains_ratio * dt_monthly * np.sin(np.pi / 180 * (0.986 * (yday - 15 - tmains_lag) + sign * 90))
        )
    ...
    df["Ambient Humidity Ratio (-)"] = psychrolib.GetHumRatioFromRelHum(
        df["Ambient Dry Bulb (C)"].values,
        df["Ambient Relative Humidity (-)"].values,
        df["Ambient Pressure (kPa)"].values * 1000,
    )
    df["Ambient Wet Bulb (-)"] = psychrolib.GetTWetBulbFromHumRatio(
        df["Ambient Dry Bulb (C)"].values,
    ...
    for hpxml_name, (category, ochre_name) in ALL_SCHEDULE_NAMES.items():
        ...
        if category == "Setpoint":
            if hpxml_name in df_norm:
                ...
                s_hpxml = create_simple_schedule(
                    ochre_dict.get("Weekday Setpoints (C)"), ochre_dict.get("Weekend Setpoints (C)")
                )
                ...
                raise OCHREException(
                    f"Must specify {ochre_name} setpoints in schedule input file or include parameters "
...
def resample_and_reindex(
    df, time_res, start_time=None, duration=None, interpolate=False, offset=None, preserve_sum=False, **kwargs
):
    ...
    if end_year > start_year:
        ...
        if len(df) % 8760 != 0:
            raise OCHREException(
                f"Simulation spans multiple years ({start_year}-{end_year}). Must provide annual data."
            )
...
def load_schedule(properties, schedule=None, time_zone=None, **house_args):
    ...
    if "HVAC Cooling Setpoint (C)" in schedule and "HVAC Heating Setpoint (C)" in schedule:
        ...
        if setpoint_diff.min() < 1:
            ...
            print("WARNING: Cooling setpoint is within 1C of heating setpoint. Separating setpoints by at least 1C.")
