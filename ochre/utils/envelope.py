...
ZONES = {"LIV": "Indoor", "FND": "Foundation", "GAR": "Garage", "ATC": "Attic"}
EXT_ZONES = {"EXT": "Outdoor", "GND": "Ground"}
...
def calculate_plane_irradiance(df, tilt, panel_azimuth, window_data=None, albedo=0.2, separate=False):
    ...
    irr = pvlib.irradiance.get_total_irradiance(
        ...
        df["GHI (W/m^2)"],
        df["DHI (W/m^2)"],
        dni_extra=df["dni_extraterrestrial"],
        airmass=df["airmass"],
        albedo=albedo,
        model="perez",
    )
...
def calculate_solar_irradiance(weather, weather_timezone, location, boundaries, **house_args):
    ...
    df = pvlib.solarposition.get_solarposition(
        time_index, latitude=location["latitude"], longitude=location["longitude"]
    )
    ...
    for bd_name, bd in boundaries.items():
        ...
        if len(areas) != len(azimuths):
            raise OCHREException(
                f"Number of areas and azimuths for {bd_name} are not equal. Areas: {areas}, Azimuths: {azimuths}"
            )

        irr = sum(
            [calculate_plane_irradiance(weather, tilt, az, window_data) * area for area, az in zip(areas, azimuths)]
        )
...
def get_boundary_rc_values(all_bd_properties, raise_error=False, **house_args):
    ...
    boundaries = load_csv(
        house_args.get("boundaries_file", "Envelope Boundaries.csv"), sub_folder="Envelope", index_col="Boundary Name"
    )
    boundary_types = load_csv(
        house_args.get("boundary_types_file", "Envelope Boundary Types.csv"), sub_folder="Envelope"
    )
    ...
    for bd_name in list(all_bd_properties.keys()):
        ...
        if r_value is not None:
            ...
            if raise_error and error_abs > 1 and error_pct > 0.15:
                raise OCHREException(
                    f"{bd_name} R value ({r_value:0.2f}) does not match closest option: {bd_type}, R={r_closest:0.2f}"
                )
            elif error_abs > 0.5 and error_pct > 0.10:
                print(
                    f"WARNING: {bd_name} R value ({r_value:0.2f}) is far from closest match: {bd_type},"
                    f" R={r_closest:0.2f}"
                )

        # Extract rows from materials file with corresponding Boundary Type
        df = materials.loc[(materials["Boundary Name"] == bd_name) & (materials["Boundary Type"] == bd_type)]
        if not len(df):
...
def calculate_film_resistances(name, boundary, location):
    ...
    t_zones = pd.Series(
        [
            location.get("Average Ground Temperature (C)", 10),
            np.nan,
...
def calculate_ashrae_infiltration_params(indoor_inf, construction, site, has_flue_or_chimney=None):
    ...
    stack_coef = (
        f_s
        * (
            convert(outside_air_density * g * infiltration_height_ft, "pounds/ft/s^2", "inch_H2O_39F")
            / (assumed_indoor_temp + 460)
        )
        ** n_i
    )  # inH2O^n/R^n
...
def calculate_ela_coefficients(zone_name, zone_height, zone_height_above_ground=0):
    ...
    f_t = (
        site_terrain_multiplier
        * ((zone_height + zone_height_above_ground) / height) ** site_terrain_exponent
    ...
    f_stack = (
        ...
        / 3.0
        * (1 + hor_lk_frac / 2.0)
        * (2.0 * neutral_level * (1.0 - neutral_level)) ** 0.5
        / (neutral_level**0.5 + (1.0 - neutral_level) ** 0.5)
    )
