...
class BoundarySurface:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # transmitted solar gain through surface to zone (for windows), in W
        self.transmitted_gain = 0
        ...
        self.iterations = kwargs["time_res"] // dt.timedelta(minutes=5) + 1
        ...
        if self.boundary_name == "Window":
            ...
            self.transmittance, self.absorptivity = utils.calculate_window_parameters(
                window_shgc, kwargs["U Factor (W/m^2-K)"], res_material
            )
        else:
            radiant_barrier = kwargs.get("Radiant Barrier", False)
            e_default = 0.05 if self.zone_name == "Attic" and radiant_barrier else 0.90
            a_default = 0.05 if self.zone_name == "Attic" and radiant_barrier else 0.60
            if is_ext_surface:
                self.emissivity = kwargs.get("Exterior Emissivity (-)", e_default)
                self.absorptivity = kwargs.get("Exterior Solar Absorptivity (-)", a_default)
                self.transmittance = 0
            else:
                self.emissivity = kwargs.get("Interior Emissivity (-)", e_default)
                self.absorptivity = kwargs.get("Interior Solar Absorptivity (-)", a_default)
    ...
    def calculate_exterior_radiation(self, t_ext, t_sky):
        ...
        # excludes irradiance
        t_surf_init = self.radiation_frac * self.t_boundary + (1 - self.radiation_frac) * t_ext
...
class Zone:
    ...
    def __init__(self, name, label, time_res, initial_schedule, **zone_args):
        ...
        inf_params = {
            ...
            "ELA": [
                "ELA (cm^2)",
                "ELA stack coefficient (L/s/cm^4/K)",
                "ELA wind coefficient (L/s/cm^4/(m/s))",
            ],
    ...
    def update_infiltration(
        ...
        # calculate component heat gains, proportional to flow rates, for
        # results only
        if sensible_flow != 0:
            self.inf_heat = sensible_gain * self.inf_flow / sensible_flow
            self.nat_vent_heat = sensible_gain * self.nat_vent_flow / sensible_flow
            self.forced_vent_heat = sensible_gain * self.forced_vent_flow / sensible_flow
        else:
            self.inf_heat = 0
            self.nat_vent_heat = 0
            self.forced_vent_heat = 0
...
class Envelope(RCModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.zones = {
            ...
            for label, name in utils.ZONES.items()
            if name in zones
        }
        ...
        if self.reduced and (self.run_internal_rad or self.run_external_rad):
            raise ModelException(
                "Cannot run non-linear radiation methods with a reduced Envelope model. "
        ...
        for bd in self.ext_boundaries:
            ...
            surface.t_prev = surface.temperature
            if "T_" + surface.node in self.state_names:
                surface.t_idx = self.state_names.index("T_" + surface.node)
            if "H_" + surface.node in self.input_names:
                surface.h_idx = self.input_names.index("H_" + surface.node)
    ...
    def initialize_state(state_names, input_names, A_c, B_c, initial_schedule, initial_temp_setpoint=None, **kwargs):
        ...
        outdoor_temp = initial_schedule["Ambient Dry Bulb (C)"]
        ground_temp = initial_schedule.get("Ground Temperature (C)", 10)
        ...
        if nones == 2 and not isinstance(initial_temp_setpoint, (int, float)):
            print(
                "WARNING: No Heating or Cooling Setpoint in schedule, and initial_temp_setpoint is not defined."
    ...
    def generate_results(self):
        ...
        if (col := "Temperature - Indoor (C)") in self.enabled_outputs:
            results[col] = self.indoor_zone.temperature
        if (col := "Unmet HVAC Load (C)") in self.enabled_outputs:
            results[col] = self.unmet_hvac_load

        # All zone temperatures (verbosity 5)
        for name, zone in self.zones.items():
            if (col := f"Temperature - {name} (C)") in self.enabled_outputs:
                results[col] = zone.temperature
        ...
        for name, zone in self.zones.items():
            if (col := f"Net Sensible Heat Gain - {name} (W)") in self.enabled_outputs:
                results[col] = self.inputs[zone.h_idx]

        if not self.linearize_infiltration:
            if (col := "Infiltration Heat Gain - Indoor (W)") in self.enabled_outputs:
                results[col] = self.indoor_zone.inf_heat
        if (col := "Forced Ventilation Heat Gain - Indoor (W)") in self.enabled_outputs:
            results[col] = self.indoor_zone.forced_vent_heat
        if (col := "Natural Ventilation Heat Gain - Indoor (W)") in self.enabled_outputs:
            results[col] = self.indoor_zone.nat_vent_heat

        # internal gains = occupancy + non-HVAC equipment only
        if (col := "Internal Heat Gain - Indoor (W)") in self.enabled_outputs:
            occupant_gain = self.current_schedule.get("Occupancy (Persons)", 0) * self.occupancy_sensible_gain
            results[col] = occupant_gain + self.indoor_zone.internal_sens_gain

        # Add window transmittance (note, gains go to indoor zone and to interior boundaries)
        if (col := "Window Transmitted Solar Gain (W)") in self.enabled_outputs:
            windows = [bd for bd in self.ext_boundaries if bd.name == "Window"]
            if windows:
                window_gain = windows[0].ext_surface.transmitted_gain
                results[col] = window_gain

        # add other component loads
        if not self.reduced:
            for bd in utils.COMPONENT_LOAD_CATEGORIES:
                if (col := f"{bd} Heat Gain - Indoor (W)") in self.enabled_outputs:
        ...
        if (col := "Occupancy (Persons)") in self.enabled_outputs:
            results[col] = self.current_schedule.get("Occupancy (Persons)", 0)

        # Add detailed heat gain results for each zone
        for name, zone in self.zones.items():
            if not self.linearize_infiltration:
                if (col := f"Infiltration Flow Rate - {name} (m^3/s)") in self.enabled_outputs:
                    results[col] = zone.inf_flow
                if (col := f"Infiltration Heat Gain - {name} (W)") in self.enabled_outputs:
                    results[col] = zone.inf_heat

            if name == "Indoor":
                if (col := f"Forced Ventilation Flow Rate - {name} (m^3/s)") in self.enabled_outputs:
                    results[col] = zone.forced_vent_flow
                if (col := f"Natural Ventilation Flow Rate - {name} (m^3/s)") in self.enabled_outputs:
                    results[col] = zone.nat_vent_flow
                if (col := f"Air Changes per Hour - {name} (1/hour)") in self.enabled_outputs:
                    air_changes = (zone.inf_flow + zone.forced_vent_flow + zone.nat_vent_flow) / zone.volume * 3600
                    results[col] = air_changes

                if (col := f"Occupancy Heat Gain - {name} (W)") in self.enabled_outputs:
                    occupant_gain = self.current_schedule.get("Occupancy (Persons)", 0) * self.occupancy_sensible_gain
                    results[col] = occupant_gain
            else:
                if zone.internal_sens_gain > 0:
                    # occupancy=0 for non-Indoor zones
                    # Only includes non-HVAC equipment
                    if (col := f"Internal Heat Gain - {name} (W)") in self.enabled_outputs:
                        results[col] = zone.internal_sens_gain

            # add radiation gain from windows and internal radiation, in W
            if self.run_internal_rad:
                if (col := f"Radiation Heat Gain - {name} (W)") in self.enabled_outputs:
                    results[col] = zone.radiation_heat

            if zone.humidity is not None:
                # Indoor is the only zone with humidity or ventilation (for now)
                if (col := f"Relative Humidity - {name} (-)") in self.enabled_outputs:
                    results[col] = zone.humidity.rh
                if (col := f"Wet Bulb - {name} (C)") in self.enabled_outputs:
                    results[col] = zone.humidity.wet_bulb
                if (col := f"Humidity Ratio - {name} (-)") in self.enabled_outputs:
                    results[col] = zone.humidity.w
                if (col := f"Net Latent Heat Gain - {name} (W)") in self.enabled_outputs:
                    results[col] = zone.humidity.latent_gains
                if (col := f"Air Density - {name} (kg/m^3)") in self.enabled_outputs:
                    results[col] = zone.humidity.density
        ...
        if self.run_external_rad:
            ...
            for bd in self.ext_boundaries:
                surface = bd.ext_surface
                if (col := f"{bd.name} Ext. Solar Gain (W)") in self.enabled_outputs:
                    results[col] = surface.solar_gain
                if (col := f"{bd.name} Ext. LWR Gain (W)") in self.enabled_outputs:
                    results[col] = surface.lwr_gain
                if (col := f"{bd.name} Ext. Surface Temperature (C)") in self.enabled_outputs:
                    results[col] = surface.temperature
                if (col := f"{bd.name} Ext. Film Coefficient (m^2-K/W)") in self.enabled_outputs:
                    results[col] = surface.res_film
        ...
        if self.run_internal_rad:
            ...
            for name, zone in self.zones.items():
                for surface in zone.surfaces:
                    ...
                    if (col := f"{bd_name} {name} LWR Gain (W)") in self.enabled_outputs:
                        results[col] = surface.lwr_gain
                    if (col := f"{bd_name} {name} Surface Temperature (C)") in self.enabled_outputs:
                        results[col] = surface.temperature
                    if (col := f"{bd_name} {name} Film Coefficient (m^2-K/W)") in self.enabled_outputs:
                        results[col] = surface.res_film
