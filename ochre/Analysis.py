...
def download_resstock_model(
    ...
    if isinstance(upgrade_id, str):
        # convert upgrade back to int
        assert "up" == upgrade_id[:2]
        upgrade_id = int(upgrade_id[2:])
    upgrade_str = f"up{upgrade_id:02}"
...
def load_eplus_file(
    file_name,
    eplus_format="ResStock",
    variable_names_file="Variable names and units.csv",
    year=2019,
):
    ...
    df_names = df_names.loc[
        df_names[f"{eplus_format} Name"].notna(),
    ...
    if eplus_format == "BEopt":
        ...
        df["HVAC Heating COP (-)"] = replace_nans(
            df["HVAC Heating Capacity (W)"]
            / 1000
            / (df["HVAC Heating Main Power (kW)"] + df.get("HVAC Heating ER Power (kW)", missing))
        )
        df["HVAC Cooling COP (-)"] = replace_nans(
            df["HVAC Cooling Capacity (W)"] / 1000 / df["HVAC Cooling Main Power (kW)"]
        )
        df["HVAC Cooling SHR (-)"] = replace_nans(
            df["HVAC Cooling Sensible Capacity (W)"] / df["HVAC Cooling Capacity (W)"]
        )
...
def calculate_metrics(results=None, results_file=None, dwelling=None, metrics_verbosity=8):
    ...
    if metrics_verbosity >= 9:
        ...
        power_names = [("Power (kW)", "Energy (kWh)"), ("Power (therms/hour)", "Energy (therms)")]
        for power_name, energy_name in power_names:
            metrics.update(
                {
                    col.replace(power_name, energy_name): results[col].sum(skipna=False)
        ...
        metrics.update(
            {
                f"Average {col}": results[col].mean()
                for col in results.columns
...
def create_comparison_metrics(
    ochre, eplus, ochre_metrics, eplus_metrics, include_mean=False, include_rmse=True, **kwargs
):
