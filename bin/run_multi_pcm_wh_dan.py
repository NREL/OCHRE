...
import shutil
import glob
from bin.compare_specific_results import (
    load_data,
    create_heat_exchanger_plots,
    create_temperature_plots,
    calculate_hot_water_delivered,
    plot_draw_event_summary,
    plot_draw_events,
    create_capacitance_plots,
    plot_pcm_enthalpies,
)
...
def set_water_heating_setpoint(df, setpoint_temp, load_profile):
    ...
    df["Water Heating Setpoint (C)"] = [
        5 if rem < 2 else setpoint_temp for rem in remainder
    ]

    return df


def update_setpoints_from_load_profile(
    schedule_df,
    load_profile_file,
    ...
    if len(schedule_df) != len(load_df):
        raise ValueError(
            "The schedule DataFrame and load profile CSV must have the same number of rows."
        )
    ...
    schedule_df["Water Heating Setpoint (C)"] = np.where(
        load_df[load_column].to_numpy() != 0, draw_setpoint, idle_setpoint
    )
...
def run_water_heater(dwelling_args, plot_title, load_profile_in, setpoint_temp):
    ...
    equipment.schedule = update_setpoints_from_load_profile(
        equipment.schedule,
        "2.00gpm_1200minStartIdle_2cycles_30minDraw_240minOff_0minEndIdle.csv",
        load_column="hot_water_dishwasher",
        idle_setpoint=setpoint_temp,
        draw_setpoint=5,
    )
...
def run_water_heater_process(
    dwelling_args, title, load_profile, setpoint_temp, filename, submission_time
):
    """
    ...
    try:
        ...
        print(
            f"{RED}{title}Error in simulation process run_water_heater: {str(e)}{RESET}"
        )
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        ...
        for setpoint_temp_c, setpoint_temp_f in zip(setpoint_temps_c, setpoint_temps_f):
            ...
            no_pcm_title = f"{no_pcm_title_base}_setpoint-{setpoint_temp_f:.0f}F_{i}"
                ...
                for pcm_vol_fraction in pcm_vol_fractions:
                    for sa_ratio in sa_ratios:
                        for h_value in h_values:
                            ...
                            model_name = f"{model_name}_SA-{sa_ratio:.2f}_H-{h_value:.2f}_setpoint-{setpoint_temp_f:.0f}F_{pcm_file_name}_{i}"
                            i += 1
    ...
    print(
        f"{BOLD}{RED}Aggregate simulation (processing) time: {total_simulation_time:.2f} seconds{RESET}"
    )
    print(
        f"{BOLD}{RED}Aggregate waiting (queue delay) time: {total_wait_time:.2f} seconds{RESET}"
    )
