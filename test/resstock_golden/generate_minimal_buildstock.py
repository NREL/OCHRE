...
ua = UpgradesAnalyzer(buildstock=str(buildstock_csv), yaml_file=str(yaml_file), opt_sat_file=str(opt_sat_file))
...
print(
    f"Generated {output_buildstock_csv} with {len(final_df)} buildings covering {len(chars_to_cover)} characteristics."
)
...
print(
    "All other characteristics are fully covered - there is at least one building for every unique value of those characteristics."
)
