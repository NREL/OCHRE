...
def create_event_schedule(start, n_events=2, event_duration=dt.timedelta(minutes=30),
                          gap=dt.timedelta(hours=4), power=1.0):
...
class ExtractEventsTestCase(unittest.TestCase):
    ...
    def test_extract_single_event(self):
        ...
        ts_data = create_time_series_schedule(
            start_time, duration, time_res,
            events=[(dt.timedelta(hours=2), dt.timedelta(hours=3), 1.5)]
        )
    ...
    def test_extract_events_with_random_offset(self):
        ...
        ts_data = create_time_series_schedule(
            start_time, duration, time_res,
            events=[(dt.timedelta(hours=2), dt.timedelta(hours=3), 1.0)]
        )
...
class ResetTimeTestCase(unittest.TestCase):
    ...
    def test_reset_to_between_events(self):
        ...
        event_schedule = create_event_schedule(
            start_time, n_events=2,
            event_duration=dt.timedelta(hours=1),
            gap=dt.timedelta(hours=5)
        )
